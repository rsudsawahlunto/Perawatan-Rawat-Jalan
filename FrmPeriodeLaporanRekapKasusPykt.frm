...
'        MsgBox "Tidak ada data antara tanggal  '" & Format(DTPickerAwal.Value, "dd - MMMM - yyyy") & "' dan '" & Format(dtpTglAkhir.Value, "dd - MMMM - yyyy") & "' ", vbInformation, "Validasi"
    ...
    strSQL = "SELECT ruangan,COUNT(ruangan) AS Jmlkasuspykt, " & _
        " SUM(JmlPasienPria) AS JmlPria,SUM(JmlPasienWanita) AS JmlWanita," & _
        " SUM(Total) AS Total From V_RekapitulasiPasienBKasusPenyakit " & _
        " WHERE TglPendaftaran BETWEEN " & _
        " '" & Format(DTPickerAwal.Value, "yyyy/MM/dd 00:00:00") & "' AND " & _
        " '" & Format(DTPickerAkhir.Value, "yyyy/MM/dd 23:59:59") & "' " & _
    ...
    With fgdata
        ...
        .Rows = intJmlRow + 2
        While rs.EOF = False
            strSQL = "SELECT ruangan,kasuspenyakit,SUM(JmlPasienPria) AS TJmlPasienPria, " & _
                " SUM(JmlPasienWanita) AS TJmlPasienWanita, " & _
                " SUM(Total) AS TTotal From V_RekapitulasiPasienBKasusPenyakit " & _
                " WHERE (TglPendaftaran BETWEEN " & _
                " '" & Format(DTPickerAwal.Value, "yyyy/MM/dd 00:00:00") & "' AND " & _
                " '" & Format(DTPickerAkhir.Value, "yyyy/MM/dd 23:59:59") & "') " & _
                ...
                " ORDER BY kasuspenyakit"
            
            Set rswilayah = Nothing
            rswilayah.Open strSQL, dbConn, adOpenForwardOnly, adLockReadOnly
            While rswilayah.EOF = False
                'baris u/ sub total
                iRowNow = iRowNow + 1
                .TextMatrix(iRowNow, 1) = rswilayah("ruangan").Value
                .TextMatrix(iRowNow, 2) = rswilayah("kasuspenyakit").Value
                .TextMatrix(iRowNow, 3) = rswilayah("TJmlPasienPria").Value
                .TextMatrix(iRowNow, 4) = rswilayah("TJmlPasienWanita").Value
                .TextMatrix(iRowNow, 5) = rswilayah("TTotal").Value
                rswilayah.MoveNext
            Wend
...
'    mdTglAkhir = dtpTglAkhir.Value
    strSQL = "SELECT * FROM V_RekapitulasiPasienBKasusPenyakit " _
        ...
        & " and kdruangan = '" & mstrKdRuangan & "' ORDER BY ruangan,kasuspenyakit"
    Set rs = Nothing
    rs.Open strSQL, dbConn, adOpenForwardOnly, adLockReadOnly
...
'             & " FROM         Instalasi where kdinstalasi = '01'", dbConn, adOpenDynamic, adLockOptimistic
'
