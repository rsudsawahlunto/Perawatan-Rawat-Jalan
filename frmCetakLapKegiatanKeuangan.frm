...
Begin VB.Form frmCetakLapKegiatanKeuangan 
   Caption         =   "Cetak Lap Kegiatan Keuangan"
   ClientHeight    =   7080
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   5865
   Icon            =   "frmCetakLapKegiatanKeuangan.frx":0000
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   ScaleHeight     =   7080
   ScaleWidth      =   5865
   ...
   End
End
Attribute VB_Name = "frmCetakLapKegiatanKeuangan"
...
Dim ReportTindakanMedis As crCetakDataPelayananTMOAApotik2a
...
Private Sub Form_Load()
    ...
    Set ReportTindakanMedis = New crCetakDataPelayananTMOAApotik2a
    Set ReportObatAlkes = New crCetakDataPelayananTMOAApotik
    Set ReportApotik = New crCetakClosingDataPelayananTMOAApotik3

    If frmLapKegiatanKeuangan.optTindakanMedis.Value = True Then
        strSQL = "SELECT TglPelayanan, JenisPasien, NoCM, NamaPasien, Umur, NamaItem, NamaDTD, NamaDiagnosa, TotalBiaya, DokterOperator, JmlHutangPenjamin, JmlTanggunganRS," & _
        " TotalBayar From V_DaftarDataPelayananLapKegiatanKeuangan" & _
        " WHERE DokterOperator like '%" & frmLapKegiatanKeuangan.dcNamaDokter.Text & "%' AND TglPelayanan BETWEEN '" & Format(frmLapKegiatanKeuangan.dtpAwal.Value, "yyyy/MM/dd hh:mm:00") & "' AND '" & Format(frmLapKegiatanKeuangan.dtpAkhir.Value, "yyyy/MM/dd hh:mm:59") & "' " & _
        " AND JenisPasien like '%" & frmLapKegiatanKeuangan.dcJenisPasien.Text & "%' AND Kelas like '%" & frmLapKegiatanKeuangan.dcKelas.Text & "%' AND JenisItem like '" & frmLapKegiatanKeuangan.dcJenisItem.Text & "%' AND NamaItem like '%" & frmLapKegiatanKeuangan.dcNamaItem.Text & "%'  AND AsalPasien LIKE '%" & frmLapKegiatanKeuangan.dcAsalPasien.Text & "%' AND KdRuangan = '" & mstrKdRuangan & "'  "
    ElseIf frmLapKegiatanKeuangan.optObatAlKes.Value = True Then
        ...
        " TotalBayar From V_DaftarDataPelayananLapKegiatanKeuangan" & _
        " WHERE TglPelayanan BETWEEN '" & Format(frmLapKegiatanKeuangan.dtpAwal.Value, "yyyy/MM/dd hh:mm:00") & "' AND '" & Format(frmLapKegiatanKeuangan.dtpAkhir.Value, "yyyy/MM/dd hh:mm:59") & "' " & _
        " AND JenisPasien like '%" & frmLapKegiatanKeuangan.dcJenisPasien.Text & "%' AND Kelas like '%" & frmLapKegiatanKeuangan.dcKelas.Text & "%' AND JenisItem like '" & frmClosingDataPelayananTM_OA_Apotik.dcJenisItem.Text & "%' AND NamaItem like '%" & frmClosingDataPelayananTM_OA_Apotik.dcNamaItem.Text & "%'  AND AsalPasien LIKE '%" & frmClosingDataPelayananTM_OA_Apotik.dcAsalPasien.Text & "%' AND KdRuangan = '" & mstrKdRuangan & "'  "
    End If
    ...
    If frmLapKegiatanKeuangan.optObatAlKes.Value = True Then
    ...
    If frmLapKegiatanKeuangan.optApotik.Value = True Then
        With ReportApotik
            .Database.AddADOCommand dbConn, dbcmd
            .txtNamaRS.SetText strNNamaRS
            .txtAlamat.SetText strNAlamatRS & ", " & strNKotaRS & " - " & strNKodepos & "  Telp. " & strNTeleponRS
...
Private Sub Form_Unload(Cancel As Integer)
    Set frmCetakLapKegiatanKeuangan = Nothing
End Sub
