...
Public Function SetJadwalClosingPosting(btnClosing As CommandButton) As Boolean
    SetJadwalClosingPosting = True
    btnClosing.Enabled = True
    dNTglBerlaku = Format(Now, "yyyy/mm/dd HH:mm:dd")
    dNTglBerlaku = CStr(Right(dNTglBerlaku, 8))
    strSQL = "SELECT JamAwal,JamAhkir FROM  JadwalClosingPosting " & _
    " where KdRuangan = '" & mstrKdRuangan & "'"
    Call msubRecFO(rs, strSQL)
    If rs.EOF = False Then
        TglAwal = rs.Fields("JamAwal")
        TglAkhir = rs.Fields("JamAhkir")
    Else
        MsgBox "Silahkan Hubungi SimRS agar dibuatkan jadwal CLOSING " & mstrNamaRuangan, vbInformation, "Informasi"
        SetJadwalClosingPosting = False
        Exit Function
    End If

    strSQL = "SELECT * FROM  JadwalClosingPosting " & _
    " where JamAwal <='" & dNTglBerlaku & "' and  JamAhkir >= '" & dNTglBerlaku & "' AND " & _
    " KdRuangan = '" & mstrKdRuangan & "'"
    Call msubRecFO(rs, strSQL)
    If rs.EOF = True Then
        MsgBox "Jadwal closing anda dari jam " & TglAwal & " sampai " & TglAkhir, vbExclamation, "Warning"
        SetJadwalClosingPosting = False
...
Public Function funcCekTglValidasi(mstrChoose As String, mobjObject As Object) As String
    Select Case mstrChoose
        Case "Tanggal"
            On Error GoTo errTanggal:
            If mobjObject.Text = "__/__/____ __:__" Then
                Exit Function
            End If
            If Mid(mobjObject.Text, 4, 2) > 12 Then
                MsgBox "Bulan tanggal tidak boleh melebihi bulan 12!", vbExclamation, "Validasi"
                mobjObject.SelStart = 3
                mobjObject.SelLength = 2
                mobjObject.SetFocus
                funcCekTglValidasi = "ErrTahun"
                Exit Function
            End If
            If CInt(Mid(mobjObject.Text, 1, 2)) > funcHitungHari(CInt(Mid(mobjObject.Text, 4, 2)), CInt(Mid(mobjObject.Text, 7, 4))) Then
                MsgBox "Hari tanggal yang dimasukkan lebih besar dari jumlah hari dari bulan tanggal yang dimasukkan!", vbExclamation, "Validasi"
                mobjObject.SelStart = 0
                mobjObject.SelLength = 2
                mobjObject.SetFocus
                funcCekTglValidasi = "ErrTahun"
                Exit Function
            End If
            If Mid(mobjObject.Text, 12, 2) > 23 Then
                MsgBox "Jam tidak boleh melebihi jam 23!", vbExclamation, "Validasi"
                mobjObject.SelStart = 11
                mobjObject.SelLength = 2
                mobjObject.SetFocus
                funcCekTglValidasi = "ErrTahun"
                Exit Function
            ...
            If Mid(mobjObject.Text, 15, 2) > 59 Then
                MsgBox "Menit tidak boleh melebihi Menit 59!", vbExclamation, "Validasi"
                mobjObject.SelStart = 14
                mobjObject.SelLength = 2
                mobjObject.SetFocus
                funcCekTglValidasi = "ErrTahun"
                Exit Function
            ...
            funcCekTglValidasi = "NoErr"
            Exit Function

errTanggal:
            MsgBox "Format Data Tanggal Salah, Format yang Benar Adalah : dd/mm/yyyy HH:mm", vbExclamation, "Validasi"
            mobjObject.SelStart = 0
            mobjObject.SelLength = Len(mobjObject.Text)
            mobjObject.SetFocus
            funcCekTglValidasi = "ErrFormat"
            Exit Function
        Case "YgLain"
    End Select
    funcCekTglValidasi = "NoErr"
...
Public Sub SetKeyPressToCharToNumber(KeyAscii As Integer)
    If KeyAscii = vbKeyBack Then Exit Sub
    If KeyAscii = 13 Then Exit Sub
    If KeyAscii = 39 Then KeyAscii = 0
    If Not ((KeyAscii >= 48 And KeyAscii <= 57) Or (KeyAscii < 48 Or KeyAscii > 57)) Then KeyAscii = 0
    If ((KeyAscii >= 123 And KeyAscii <= 126)) Then KeyAscii = 0
End Sub
...
Public Sub msubPesanCetak()
    If MsgBox("Apakah Anda Ingin Langsung Mencetak Laporan?" & vbNewLine _
        & "Pilih No Jika Ingin Ditampikan Terlebih Dahulu", vbInformation + vbYesNo, "Konfirmasi") = vbYes Then
        vCetakLaporan = "Print"
    Else
...
Public Function FormatPembulatan(Value As Double, KodeInstalasi As String) As Double
    Call msubRecFO(rsL, "select * from SettingDataPendukung where kdInstalasi='" & KodeInstalasi & "'")
    Dim tempJumlahKoma As String
    Dim i As Integer
    Dim valueJumlahPembulatan As Integer
    If (rsL.EOF = False) Then
        valueJumlahPembulatan = rsL("JmlPembulatanHarga").Value
        If (valueJumlahPembulatan <> 0) Then
            tempJumlahKoma = "."
        End If
     Else
        valueJumlahPembulatan = 0
    End If
    
    For i = 1 To valueJumlahPembulatan
        tempJumlahKoma = tempJumlahKoma + "#"
    Next i
    
    If tempJumlahKoma = "" Then tempJumlahKoma = 0
    If (Value = 0) Then
        FormatPembulatan = Value
    Else
        FormatPembulatan = Format(Value, "#,###" + tempJumlahKoma)
    End If
End Function
