...
Begin VB.Form frmCetakLapDataBumil 
   Caption         =   "Cetak Lap Data Bumil"
   ClientHeight    =   7065
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   5865
   Icon            =   "frmCetakLapDataBumil.frx":0000
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   ScaleHeight     =   7065
   ScaleWidth      =   5865
   ...
   End
End
Attribute VB_Name = "frmCetakLapDataBumil"
...
Dim Report As New crLapDataBumil
...
Private Sub Form_Load()
    ...
    If mblnStatusCetakRD = True Then
        adocomd.CommandText = "select * from V_DataIbuHamil  WHERE TglPeriksa BETWEEN '" & Format(mdTglAwal, "yyyy/MM/dd 00:00:00") & "' AND '" & Format(mdTglAkhir, "yyyy/MM/dd 23:59:59") & "'"
    Else
        adocomd.CommandText = "select * from V_DataIbuHamil  WHERE TglPeriksa BETWEEN '" & Format(mdTglAwal, "yyyy/MM/dd 00:00:00") & "' AND '" & Format(mdTglAkhir, "yyyy/MM/dd 23:59:59") & "'"
    End If
    ...
    With Report
        ...
        .usG.SetUnboundFieldSource ("{ado.Gravite}")
        .usP.SetUnboundFieldSource ("{ado.Partus}")
        .usA.SetUnboundFieldSource ("{ado.Abortus}")
        .usumurkehamilan.SetUnboundFieldSource ("{ado.umur kehamilan}")
        .usbb.SetUnboundFieldSource ("{ado.berattinggibadan}")
        .ustd.SetUnboundFieldSource ("{ado.tekanandarah}")
        .usimunisasi.SetUnboundFieldSource ("{ado.imunisasi}")
...
Private Sub Form_Unload(Cancel As Integer)
    Set frmCetakLapDataBumil = Nothing
End Sub
