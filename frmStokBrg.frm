...
On Error GoTo Errload
    ...
    If bolStatusFIFO = True Then
        strNoTerima = dgBrg.Columns("NoTerima").Value
    End If
    
    With frmPemesananBarang
        .Show
        
        If mstrKdInstalasiLogin = "07" And mstrKdRuangan <> "701" Then
            .dcRuanganTujuan.BoundText = "701"
        ElseIf mstrKdInstalasiLogin = "02" Then
...
'            strSQLx = "execute CariBarangNStokMedis_V '" & .dgObatAlkes.Columns("KdBarang") & "','" & .dcRuanganTujuan.BoundText & "'"
'            Call msubRecFO(rsx, strSQLx)
'            If rsx.EOF = False Then
            .txtStock.Text = dbRst("JmlStokOnHand").Value
'            Else
'            .txtStock.Text = "0"
'            End If
'            .txtNamaBarang.Text = ""
...
'    End If
    ...
    If mstrKdInstalasiLogin = "07" Then
        chkRuangan.Visible = True: dcRuangan.Visible = True
    Else
        
        chkRuangan.Visible = False: dcRuangan.Visible = False
    End If
...
Sub subLoadGridSource()
    ...
    If bolStatusFIFO = True Then
        If dcRuangan.Text = "" Then
            strSQL = "SELECT * " & _
                     " FROM V_AmbilStockBarangFIFO " & _
                     " WHERE [Nama Barang] LIKE '%" & txtCariBarang & "%' AND [Jenis Barang] LIKE '%" & txtCariJenisBarang & "%' AND [Asal Barang] LIKE '%" & txtCariAsalBarang & "%' AND KdRuangan = '" & mstrKdRuangan & "' order by [Nama Barang]"
        Else
            strSQL = "SELECT * " & _
                     " FROM V_AmbilStockBarangFIFO " & _
                     " WHERE [Nama Barang] LIKE '%" & txtCariBarang & "%' AND [Jenis Barang] LIKE '%" & txtCariJenisBarang & "%' AND [Asal Barang] LIKE '%" & txtCariAsalBarang & "%' AND KdRuangan = '" & dcRuangan.BoundText & "' order by [Nama Barang]"
        End If
    Else
        If dcRuangan.Text = "" Then
            strSQL = "SELECT * " & _
