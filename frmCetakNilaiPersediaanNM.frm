...
Private Sub Form_Load()
    ...
    ElseIf mstrKdKelompokBarang = "01" Then 'non medis
        With Report1
            ...
            .txtPeriodeClosing.SetText "Periode Closing " & frmNilaiPersediaanNM.dcNoClosing.BoundText
    ...
    If frmNilaiPersediaanNM.ChkPerjenis.Value = 0 Then
        'AND NoRegisterAsset <> '0'
        strSQL = "SELECT * FROM V_DataStokBarangNonMedisRekapx " & _
        " WHERE ((NoRegisterAsset <> '0000000'  AND NoRegisterAsset <>'000000') OR KdJenisAset is null) and KdRuangan = '" & mstrKdRuangan & "' AND (TglClosing = '" & Format(frmNilaiPersediaanNM.dcNoClosing.BoundText, "yyyy/MM/dd hh:mm:ss") & "') AND StokReal<> 0" & _
        " ORDER By JenisBarang, NamaBarang"
    Else
        'AND NoRegisterAsset <> '0'
        strSQL = "SELECT * FROM V_DataStokBarangNonMedisRekapx " & _
        " WHERE ((NoRegisterAsset <> '0000000'  AND NoRegisterAsset <>'000000') OR KdJenisAset is null) and JenisBarang like '" & frmNilaiPersediaanNM.dcJenisBarang.Text & "%' and KdRuangan = '" & mstrKdRuangan & "' AND (TglClosing = '" & Format(frmNilaiPersediaanNM.dcNoClosing.BoundText, "yyyy/MM/dd HH:mm:ss") & "')" & _
        " ORDER By JenisBarang, NamaBarang"
    End If
