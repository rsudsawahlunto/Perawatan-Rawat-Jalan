...
Begin VB.Form frmOrderPelayanan 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Medifirst2000 - Pesan Pelayanan Tindakan Medis dan Obat  Alkes"
   ClientHeight    =   7710
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   13215
   ...
   EndProperty
   Icon            =   "frmOrderPelayanan.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "frmOrderPelayanan.frx":0CCA
      ...
      TabPicture(1)   =   "frmOrderPelayanan.frx":0CE6
      ...
      Begin VB.ComboBox cbCito 
         Appearance      =   0  'Flat
         Height          =   330
         ItemData        =   "frmOrderPelayanan.frx":0D02
         Left            =   -70440
         List            =   "frmOrderPelayanan.frx":0D0C
      ...
      Begin VB.Frame Frame8 
         ...
         Begin VB.ComboBox cbCitoOA 
            Appearance      =   0  'Flat
            Height          =   330
            ItemData        =   "frmOrderPelayanan.frx":0D19
            Left            =   4320
            List            =   "frmOrderPelayanan.frx":0D23
            Style           =   2  'Dropdown List
            TabIndex        =   44
            Top             =   0
            Visible         =   0   'False
   ...
   Begin VB.Image Image2 
      Height          =   945
      Left            =   11400
      Picture         =   "frmOrderPelayanan.frx":0D30
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1875
   End
   Begin VB.Image Image3 
      Height          =   975
      Left            =   1800
      Picture         =   "frmOrderPelayanan.frx":1AB8
      Stretch         =   -1  'True
      Top             =   0
      Width           =   13095
   End
   Begin VB.Image Image1 
      Height          =   975
      Left            =   0
      Picture         =   "frmOrderPelayanan.frx":3116
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1800
   End
End
Attribute VB_Name = "frmOrderPelayanan"
...
'                If dcNamaPelayanan.BoundText = "" Then MsgBox "Data barang yang akan dipesan harus diisi", vbExclamation, "Validasi": Exit Sub
                ...
                For i = 1 To fgData.Rows - 1
                    If fgData.TextMatrix(i, 0) = "" Then MsgBox "Cek R/ke kosong.", vbExclamation, "Validasi": Exit Sub
                    If fgData.TextMatrix(i, 4) = "" Or fgData.TextMatrix(i, 4) = 0 Then MsgBox "Cek Jumlah Barang ", vbExclamation, "Validasi": Exit Sub
                Next i
...
Private Sub dcNamaPelayanan_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        If Len(Trim(dcNamaPelayanan.Text)) = 0 Then
           fgData.TextMatrix(fgData.Row, 8) = dcNamaPelayanan.Text
           fgData.TextMatrix(fgData.Row, 9) = dcNamaPelayanan.BoundText
        ...
        If dcNamaPelayanan.MatchedWithList = True Then Call dcNamaPelayanan_Change
         strSQL = "SELECT TOP (200) KdPelayananRS, NamaPelayanan" & _
         " FROM V_ListPelayanan Where namapelayanan like '%" & dcNamaPelayanan.Text & "%'"
         Call msubDcSource(dcNamaPelayanan, rs, strSQL)
'        Call msubRecFO(rs, strSQL)
        If rs.EOF = True Then dcNamaPelayanan = "": dcNamaPelayanan.Visible = True: Exit Sub
...
'        Call msubRecFO(rs, strSQL)
        ...
        If rs.EOF = True Then dcRuanganTujuan = "": Exit Sub
...
Private Sub dcRuanganTujuan_KeyUp(KeyCode As Integer, Shift As Integer)
If KeyCode <> 40 And KeyCode <> 38 Then
        strSQL = "select KdRuangan, NamaRuangan from Ruangan WHERE (NamaRuangan LIKE '%" & dcRuanganTujuan.Text & "%') and StatusEnabled=1"
        'Call msubRecFO(rs, strSQL)
        Call msubDcSource(dcRuanganTujuan, rs, strSQL)
...
'        Call msubRecFO(rs, strSQL)
        ...
        If rs.EOF = True Then dcRuanganTujuanTM = "": Exit Sub
...
Private Sub dcRuanganTujuanTM_KeyUp(KeyCode As Integer, Shift As Integer)
If KeyCode <> 40 And KeyCode <> 38 Then
        strSQL = "select KdRuangan, NamaRuangan from Ruangan WHERE (NamaRuangan LIKE '%" & dcRuanganTujuanTM.Text & "%') and StatusEnabled=1"
        'Call msubRecFO(rs, strSQL)
        Call msubDcSource(dcRuanganTujuanTM, rs, strSQL)
...
Private Sub dcRuanganTujuanTM_LostFocus()
        strSQL = "select KdRuangan, NamaRuangan from Ruangan WHERE (NamaRuangan LIKE '%" & dcRuanganTujuanTM.Text & "%') and StatusEnabled=1"
        'Call msubRecFO(rs, strSQL)
        Call msubDcSource(dcRuanganTujuanTM, rs, strSQL)
        If rs.EOF = True Then dcRuanganTujuanTM = "": Exit Sub
...
Private Sub dgObatAlkes_DblClick()
    ...
    For i = 0 To fgData.Rows - 1
        If dgObatAlkes.Columns("KdBarang") = fgData.TextMatrix(i, 2) Then
    ...
    With fgData
        ...
        .TextMatrix(.Row, 4) = "1"
        .TextMatrix(.Row, 5) = "No"
        .TextMatrix(.Row, 11) = dgObatAlkes.Columns("KdAsal")
...
Private Sub dgPelayananRS_LostFocus()
'dgPelayananRS.Visible = False
End Sub
    ...
    Select Case KeyCode
        Case 13
            If fgData.Col = fgData.Cols - 1 Then
                If fgData.TextMatrix(fgData.Row, 4) <> "" Then
                    If fgData.TextMatrix(fgData.Rows - 1, 4) <> "" Then
                        fgData.Rows = fgData.Rows + 1
                    End If
                    fgData.Row = fgData.Rows - 1
                    fgData.TextMatrix(fgData.Row, 0) = "1"
                    fgData.Col = 0
                Else
                    fgData.Col = 0
                End If
            Else
                For i = 0 To fgData.Cols - 1
...
Private Sub fgData_KeyPress(KeyAscii As Integer)
    ...
    Select Case fgData.Col
        ...
        Case 5 'keterangan
            Call subLoadDataCombo(cbCitoOA)
        ...
        Case 7 'keterangan pakai
            txtIsi.MaxLength = 50
            Call subLoadText
            txtIsi.Text = Chr(KeyAscii)
            txtIsi.SelStart = Len(txtIsi.Text)

        Case 8 'keterangan lain
            Call subLoadDataCombo(dcNamaPelayanan)
    ...
    Select Case SSTab1.Tab
        ...
        Case 1
            With fgData
                ...
                .TextMatrix(0, 2) = "Kode Barang"
                .TextMatrix(0, 3) = "Nama Barang"
                .TextMatrix(0, 4) = "Jumlah"
                .TextMatrix(0, 5) = "Cito"
                .TextMatrix(0, 6) = "Aturan Pakai"
                .TextMatrix(0, 7) = "Keterangan Pakai"
                .TextMatrix(0, 8) = "Nama Pelayanan RS"
                .TextMatrix(0, 9) = "KdPelayananRSUsed"
                .TextMatrix(0, 10) = "Keterangan Lainnya"
...
'    " FROM V_RuanganOrderTMOA " _
'    & " where Fungsi Not IN ('Penunjang Non Medik','Penunjang Non Medis') Order by NamaRuangan"
    
    strSQL = "SELECT KdRuangan, NamaRuangan" & _
    " FROM V_RuanganOrderTMOA Order by NamaRuangan"
    
    Call msubDcSource(dcRuanganTujuanTM, rs, strSQL)
    Call msubDcSource(dcRuanganTujuan, rs, strSQL)

    strSQL = "SELECT TOP (200) KdPelayananRS, NamaPelayanan" & _
    " FROM V_ListPelayanan"
    Call msubDcSource(dcNamaPelayanan, rs, strSQL)
...
Private Sub txtIsiTM_Change()
    ...
    Select Case fgDataTM.Col
        ...
        Case 0 ' nama barang
            ...
            " FROM V_DetailPelayananMedisNew where NamaPelayanan like '" & txtIsiTM.Text & "%' and KdRuangan like '%" & dcRuanganTujuanTM.BoundText & "%' and KdKelas like '%" & mstrKdKelas & "%'  and StatusEnabled=1 "

            Call msubRecFO(dbRst, strSQL)
            
'            If dbRst.EOF = True Then
'                MsgBox "Data Pelayanan Medis Tidak Di Temukan.", vbInformation, "Validasi"
'                Exit Sub
'            End If
...
Private Sub txtNamaDokterTM_Change()
...
Private Function sp_DetailOrderPelayananOA(f_KdBarang As String, f_KdAsal As String, f_JmlBarang As Integer, f_ResepKe As Integer, f_AturanPakai As String, f_KeteranganPakai As String, f_KdPelayananRSUsed As String, f_KeteranganLainnya As String, f_Cito As String) As Boolean
    ...
    With dbcmd
        ...
        .Parameters.Append .CreateParameter("NoCM", adChar, adParamInput, 6, txtNoCMTM.Text)
