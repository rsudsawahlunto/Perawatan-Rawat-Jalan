...
Begin VB.Form frmNilaiPersediaan 
   ...
   EndProperty
   Icon            =   "frmNilaiPersediaan.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ...
   Begin VB.Image Image2 
      Height          =   945
      Left            =   12960
      Picture         =   "frmNilaiPersediaan.frx":0CCA
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1875
   End
   Begin VB.Image Image3 
      Height          =   975
      Left            =   0
      Picture         =   "frmNilaiPersediaan.frx":1A52
      Stretch         =   -1  'True
      Top             =   0
      Width           =   1800
   End
   Begin VB.Image Image1 
      Height          =   975
      Left            =   1800
      Picture         =   "frmNilaiPersediaan.frx":4413
      Stretch         =   -1  'True
      Top             =   0
      Width           =   13335
   End
   ...
   End
End
Attribute VB_Name = "frmNilaiPersediaan"
...
Private Sub subLoadDcSource()
    ...
    strSQL = "SELECT DISTINCT  TglClosing, TglClosing AS Alias FROM Closing WHERE KdRuangan = '" & mstrKdRuangan & "' ORDER BY TglClosing"
    Call msubDcSource(dcNoClosing, rs, strSQL)

    strSQL = "SELECT KdDetailJenisBarang, DetailJenisBarang FROM V_S_DetailJenisBarangMedis Order By DetailJenisBarang"
...
Private Sub subLoadGridSource()
    ...
    If ChkPerjenis.Value = 0 Then
        strSQL = "SELECT * FROM V_DataStokBarangMedisRekapFifo " & _
    ...
    Else
        ...
        strSQL = "SELECT * FROM V_DataStokBarangMedisRekapFifo " & _
        " WHERE JenisBarang like '" & dcJenisBarang.Text & "%' and KdRuangan = '" & mstrKdRuangan & "' AND (TglClosing = '" & Format(dcNoClosing.BoundText, "yyyy/MM/dd HH:mm:ss") & "')  AND StokReal<> 0" & _
        " ORDER By JenisBarang, NamaBarang"
    End If
    ...
    For i = 1 To rs.RecordCount
        With fgData
            ...
            .TextMatrix(i, 2) = IIf(IsNull(rs("KeKuatan")), "", rs("KeKuatan"))
            ...
            .TextMatrix(i, 6) = IIf(IsNull(rs("TglKadaluarsa")), "__/__/____", Format(rs("TglKadaluarsa"), "dd/MM/yyyy"))
            .TextMatrix(i, 7) = Format(rs("HargaNetto1"), "#,###.00")
            .TextMatrix(i, 8) = Format(rs("HargaNetto2"), "#,###.00")
            .TextMatrix(i, 9) = Format(rs("Discount"), "#,###")
            .TextMatrix(i, 10) = Format((rs("StokReal") * rs("HargaNetto1")), "#,###.00")
            txtTotalReal.Text = CCur(txtTotalReal.Text) + (rs("StokReal") * rs("HargaNetto1"))
            .TextMatrix(i, 11) = rs("KdBarang")
            .TextMatrix(i, 12) = rs("KdAsal")

            .TextMatrix(i, 13) = IIf(IsNull(rs("Lokasi")), "", rs("Lokasi"))
            .TextMatrix(i, 14) = IIf(IsNull(rs("NoTerima")), "", rs("NoTerima"))
            .TextMatrix(i, 15) = IIf(IsNull(rs("TglInputStok")), "__/__/____ __:__", Format(rs("TglInputStok"), "dd/MM/yyyy HH:mm"))
        End With
        rs.MoveNext
        fgData.Rows = fgData.Rows + 1
    Next i
    txtTotalStokReal.Text = txtTotalStokReal.Text 'Format(txtTotalStokReal.Text, "#,##0.0#")
...
Private Sub subLoadGridSourceNM()
    On Error GoTo Errload
    Dim i As Integer

    If ChkPerjenis.Value = 0 Then
        strSQL = "SELECT * FROM V_DataStokBarangNonMedisRekap " & _
        " WHERE KdRuangan = '" & mstrKdRuangan & "' AND (TglClosing = '" & Format(dcNoClosing.BoundText, "yyyy/MM/dd 00:00:00") & "')" & _
        " ORDER By JenisBarang, NamaBarang"
    Else
        If Periksa("datacombo", dcJenisBarang, "Jenis barang kosong") = False Then Exit Sub
        strSQL = "SELECT * FROM V_DataStokBarangNonMedisRekap " & _
        " WHERE JenisBarang ='" & dcJenisBarang.BoundText & "' and KdRuangan = '" & mstrKdRuangan & "' AND (TglClosing = '" & Format(dcNoClosing.BoundText, "yyyy/MM/dd 00:00:00") & "')" & _
        " ORDER By JenisBarang, NamaBarang"
    End If
    ...
    For i = 1 To rs.RecordCount
        With fgData
            ...
            .TextMatrix(i, 3) = rs("Merk")
            .TextMatrix(i, 4) = rs("Type")
            .TextMatrix(i, 5) = rs("Bahan")
            .TextMatrix(i, 6) = rs("StokSystem")
            .TextMatrix(i, 7) = rs("StokReal")
            txtTotalStokReal.Text = CCur(txtTotalStokReal.Text) + rs("StokReal")
            .TextMatrix(i, 8) = Format(rs("HargaNetto"), "#,###.00")
            .TextMatrix(i, 9) = Format(rs("Discount"), "#,###")
            .TextMatrix(i, 10) = Format((rs("StokReal") * rs("HargaNetto")), "#,###.00")
            txtTotalReal.Text = CCur(txtTotalReal.Text) + (rs("StokReal") * rs("HargaNetto"))
            .TextMatrix(i, 11) = rs("KdBarang")
            .TextMatrix(i, 12) = rs("KdAsal")
            .TextMatrix(i, 13) = rs("KdMerk")
            .TextMatrix(i, 14) = rs("KdType")
            .TextMatrix(i, 15) = rs("KdBahanBarang")
            .TextMatrix(i, 16) = IIf(IsNull(rs("Lokasi")), "", rs("Lokasi"))
        End With
        rs.MoveNext
        fgData.Rows = fgData.Rows + 1
    Next i
    txtTotalStokReal.Text = Format(txtTotalStokReal.Text, "#,###")
    txtTotalReal.Text = Format(txtTotalReal.Text, "#,###.00")
...
Private Sub subSetGrid()
    ...
    With fgData
        ...
        .ColWidth(5) = 900
        .ColWidth(6) = 1200
        If chkTampilHarga.Value = Checked Then
            .ColWidth(7) = 0
            .ColWidth(8) = 1800
            .ColWidth(10) = 1450
        Else
...
Private Sub chkTampilHarga_Click()
    On Error GoTo Errload
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    If KeyCode = vbKeyF1 Then
        frmCetakNilaiPersediaan.chkHarga = chkTampilHarga.Value
        frmCetakNilaiPersediaan.Show
    End If
