...
Begin VB.Form frmCetakLapSaldoBarang 
   Caption         =   "Cetak Saldo Barang"
   ClientHeight    =   7065
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   5850
   Icon            =   "frmCetakLapSaldoBarang.frx":0000
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   ScaleHeight     =   7065
   ScaleWidth      =   5850
   ...
   End
End
Attribute VB_Name = "frmCetakLapSaldoBarang"
...
Dim Report As New crLapSaldoBarang
...
Private Sub Form_Load()
    ...
    Judul2 = "LAPORAN SALDO BARANG MEDIS (PERBULAN)"
    Judul3 = "LAPORAN SALDO BARANG MEDIS (PERTAHUN)"
...
Private Sub Form_Unload(Cancel As Integer)
    Set frmCetakLapSaldoBarang = Nothing
End Sub
...
Private Sub LaporanPerHari()
    ...
    If strGroup = "" Then
        adocomd.CommandText = "SELECT * FROM V_DataTransaksiBarangMedis " & _
    ...
    With Report
        ...
        .unAwal.SetUnboundFieldSource ("{ado.JmlStokAwal}")
        .unMasuk.SetUnboundFieldSource ("{ado.JmlTerima}")
        .unKeluar.SetUnboundFieldSource ("{ado.JmlKeluar}")
        .unSaldo.SetUnboundFieldSource Format(("{ado.JmlStokAwal} + {ado.JmlTerima} - {ado.JmlKeluar}"), "###,##0")
        .ucNetto.SetUnboundFieldSource Format(("{ado.HargaNetto}"), "###,##0")
        .ucTotal.SetUnboundFieldSource Format(("{ado.JmlStokAwal} + {ado.JmlTerima} - {ado.JmlKeluar} * {ado.HargaNetto}"), "###,##0")
    End With
End Sub
...
Private Sub LaporanPerBulan()
    ...
    If strGroup = "" Then
        adocomd.CommandText = "SELECT {fn MONTHNAME (TglTransaksi)} AS TglTransaksi, Pabrik, AsalBarang, DetailJenisBarang, StatusBarang, GolonganBarang, NamaBarang, JmlStokAwalPB, JmlTerima, JmlKeluar, HargaNetto FROM V_DataTransaksiBarangMedis " & _
...
Private Sub LaporanPerTahun()
    ...
    If strGroup = "" Then
        adocomd.CommandText = "SELECT *, {fn YEAR (TglTransaksi) } AS TglTransaksi, Pabrik, AsalBarang, DetailJenisBarang, StatusBarang, GolonganBarang, NamaBarang, JmlStokAwalPB, JmlTerima, JmlKeluar, HargaNetto FROM V_DataTransaksiBarangMedis " & _
